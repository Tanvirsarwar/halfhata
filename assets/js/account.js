...
function render() {
  ...
  if (!u) {
    body.innerHTML = `<div class="panel" style="text-align:center;padding:44px">
      <h2 style="font-size:22px">Sign in to your account</h2>
      <p style="color:var(--muted);margin:8px 0 20px">See your orders, delivery status and notifications.</p>
      <div class="oauth" style="max-width:420px;margin:0 auto">
        <button class="btn" id="g" style="border:1px solid var(--line);background:#fff;padding:14px">${ic('google',18)} Continue with Google</button>
        <button class="btn" id="c" style="border:1px solid var(--line);background:#fff;padding:14px">${ic('user',18)} Create Account</button>
      </div></div>`;
    document.getElementById('g').onclick = () => { const email = prompt('Your Google email'); if (email === null) return; const name = (email||'').split('@')[0] || 'Customer'; Store.setUser({ name, email:(email||'').trim() }); render(); };
    document.getElementById('c').onclick = () => { const n = prompt('Full name'); if(!n) return; Store.setUser({ name:n, email:prompt('Email')||'' }); render(); };
    return;
  }
